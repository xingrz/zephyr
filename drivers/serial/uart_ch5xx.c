...
static int uart_ch5xx_init(const struct device *dev)
{
	...
	ret = clock_control_on(CH5XX_CLOCK_CONTROLLER, (clock_control_subsys_t)cfg->ccfg);
	if (ret < 0) {
		LOG_ERR("Failed to turn on clock: %d", ret);
		return ret;
	}

	clock_control_get_rate(CH5XX_CLOCK_CONTROLLER, (clock_control_subsys_t)cfg->ccfg,
			       &clk_freq);
	LOG_DBG("clk_freq: %d", clk_freq);

	regval = 10 * clk_freq * 2 / 16 / cfg->baud_rate;
	regval = (regval + 5) / 10;
