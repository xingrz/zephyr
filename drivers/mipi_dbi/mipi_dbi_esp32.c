...
#define DT_DRV_COMPAT espressif_esp32_lcd_cam_out
...
static int mipi_dbi_esp32_config_dma(const struct device *dev, const uint8_t *buffer, size_t len)
{
	...
	data->dma_block.source_address = (uint32_t)buffer;
...
static int mipi_dbi_esp32_start_dma(const struct device *dev)
{
	const struct mipi_dbi_esp32_config *cfg = dev->config;
	int ret;
...
{
	uint16_t index = dbi_config->config.slave;
				  ...
				  bool active)
{
	...
	if (spi_cs_is_gpio(&dbi_cfg->config) && dbi_cfg->config.cs.gpio.port != NULL) {
		if (!gpio_is_ready_dt(&dbi_cfg->config.cs.gpio)) {
			LOG_ERR("CS GPIO port %s pin %d is not ready",
				dbi_cfg->config.cs.gpio.port->name, dbi_cfg->config.cs.gpio.pin);
			return;
		}

		if (active) {
			k_busy_wait(dbi_cfg->config.cs.delay);
		}

		gpio_pin_set_dt(&dbi_cfg->config.cs.gpio, active);
...
static void mipi_dbi_esp32_clear_sem(struct k_sem *sem)
{
				   ...
				   bool send_cmd, uint8_t cmd, const uint8_t *buffer, size_t len)
{
	...
	if (len > 0 && !esp_ptr_dma_capable((void *)buffer)) {
	...
	mipi_dbi_esp32_clear_sem(&data->dma_sem);
	...
	if (len > 0) {
		ret = mipi_dbi_esp32_config_dma(dev, tmp_buffer ? tmp_buffer : buffer, len);
		if (ret < 0) {
			goto free;
		}

		ret = mipi_dbi_esp32_start_dma(dev);
		if (ret < 0) {
			goto free;
		}
	}
